#ifndef VIATRA__QUERY__SAFETYLOGIC__TRAIN_HITS_ANOTHER_TRAIN_NEXT_AFTER_NEXT_FRAME_68_H_
#define VIATRA__QUERY__SAFETYLOGIC__TRAIN_HITS_ANOTHER_TRAIN_NEXT_AFTER_NEXT_FRAME_68_H_
...
class TrainHitsAnotherTrainNextAfterNextFrame_68Vector;

struct TrainHitsAnotherTrainNextAfterNextFrame_68 {
	
	using PBFrame = PB_TrainHitsAnotherTrainNextAfterNextFrame_68;
	using FrameVector = TrainHitsAnotherTrainNextAfterNextFrame_68Vector;
	...
	bool _7 = false;
	::railRoadModel::ITurnout* _8 = nullptr;
	...
	std::string toString() const
	{
		...
		ret += Viatra::Query::Util::Convert::ToString(_11);
	...
	{
		PB_TrainHitsAnotherTrainNextAfterNextFrame_68 pbframe;
	...
	void ParseFromString(std::string str, ModelRoot *mr)
	{
		...
		_8 = (pbframe._8() == -1) 
			? nullptr 
			: dynamic_cast<::railRoadModel::ITurnout*>(mr->findModelElementByID(pbframe._8()));
...
class TrainHitsAnotherTrainNextAfterNextFrame_68Vector 
	: private std::vector<TrainHitsAnotherTrainNextAfterNextFrame_68>
{
	public:
	using std::vector<TrainHitsAnotherTrainNextAfterNextFrame_68>::push_back;
	using std::vector<TrainHitsAnotherTrainNextAfterNextFrame_68>::clear;
	using std::vector<TrainHitsAnotherTrainNextAfterNextFrame_68>::empty;
	using std::vector<TrainHitsAnotherTrainNextAfterNextFrame_68>::size;
	using std::vector<TrainHitsAnotherTrainNextAfterNextFrame_68>::begin;
	using std::vector<TrainHitsAnotherTrainNextAfterNextFrame_68>::end;
	...
	{
		PB_TrainHitsAnotherTrainNextAfterNextFrame_68Vector pbFrameVector;
	...
	std::string SerializeAsString()
	{
		...
		for(auto& storedVector: *this){
			...
			pbFrame.set__11(storedVector._11);
...
#endif /*  VIATRA__QUERY__SAFETYLOGIC__TRAIN_HITS_ANOTHER_TRAIN_NEXT_AFTER_NEXT_FRAME_68_H_ */
