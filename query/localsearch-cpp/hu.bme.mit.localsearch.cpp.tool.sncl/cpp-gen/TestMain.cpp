 ...
 */
 ...
 //       auto deletedName = deletableSN->name();
 //       for(auto cl : clStore)
 //         if(cl->cnn() == deletableSN) cl->set_cnn(nullptr);
 //
 //       for(auto sn : snStore)
 ...
 //            if(match.C1->name() == "C6" && match.C2->name() == "C5") FAIL()
 //            << "Not properly handled one SN's delete. SN.name = " << deletedName;
 //          }
 //        }catch(...){
 ...
 //         if(!SNCLIteratorTest::validMatch(m)) FAIL() << "Unbound LinkedQuery found an invalid match: " << m.S1->name()
 //          << "-" << m.S2->name();
 //       }
 //       int i = 0;
 ...
 //          for(auto m : matches){
 //            if(!SNCLIteratorTest::validMatch(m)) FAIL() << "Bound LinkedQuery found an invalid match: "
 //            << m.S1->name() << "-" << m.S2->name();
 //          }
 //        }
 //        else{
 ...
 //          if(matches.size() > 0)
 //            for(auto m : matches){
 //              if(!SNCLIteratorTest::validMatch(m)) FAIL() << "Bound LinkedQuery found an invalid match: " << m.S1->name()
 //              << "-" << m.S2->name();
 //            }
 //          else{
 ...
 //             auto str = m.C1->name();
 //             if(str != "C1" && str != "C2" && str != "C3" && str != "C4") FAIL() << "CountLinks Init error: "
 //             << m.C1->name() << " numOfSN=" << m.numOfSN << " numOfCL=" << m.numOfCL;
 //           }else if(m.numOfCL == 2){ //C5,C6
 //             auto str = m.C1->name();
 //             if(str != "C5" && str != "C6") FAIL() << "CountLinks Init error: " << m.C1->name()
 //             << " numOfSN=" << m.numOfSN << " numOfCL=" << m.numOfCL;
 //           }
 //         }else{
 ...
 //       ::arch::ICL* c2 = nullptr;
 //       auto c7 = new ::arch::LocalCL(100); c7->set_name("C7");
 ...
 //         else if(str != "C5" && str != "C6") FAIL() << "CountLinks CL Modified  error: " << m.C1->name() << " numOfSN=" << m.numOfSN << " numOfCL=" << m.numOfCL;
 //       }
 //       //Test modification - new SN
 //       auto s2 = c2->cnn();
 //       auto s6 = new ::arch::LocalSN(101); s6->set_name("S6");
 //       snStore.push_back(s6);
 //       std::vector< ::arch::ISN* > temp = s2->link();
 //       temp.push_back(s6);
 //       s2->set_link(temp);
 //       matches = countLinksMatcher.matches();
 //       ASSERT_EQ(matches.size(), clStore.size());
 ...
 //         auto str = m.C1->name();
 //         if(str == "C2" || str == "C7"){
 //             if(m.numOfSN == 3 && m.numOfCL == 5);
 //             else FAIL() << "CountLinks SN Modified error: " << m.C1->name() << " numOfSN=" << m.numOfSN << " numOfCL=" << m.numOfCL;
 //         }
 //      }
 //     }
 ...
 //       for(auto& m : matches){
 //         if(m.C1->cnn()->name() != "S5") FAIL() << "IsolatedSN error: Found an unexpected match: "
 //         << m.C1->name() << "--" << m.C1->cnn()->name();
 //       }
 //       auto s6 = new ::arch::LocalSN(100); s6->set_name("S6");
 //       snStore.push_back(s6);
 //       ::arch::ISN* s5 = nullptr;
 //       for(auto sn : snStore){
 //         if(sn->name() == "S5") s5 = sn;
 ...
 //       std::vector< ::arch::ISN* > temp = s6->link();
 //       temp.push_back(s5);
 //       s6->set_link(temp);
 ...
 //       ASSERT_EQ(matches.size(), 2);
 //       for(auto cl : clStore) cl->set_countReachableSN(2);
 //       matches = numEqualWithMemberMatcher.matches();
 //       ASSERT_EQ(matches.size(), 4);
 //       for(auto cl : clStore) cl->set_countReachableSN(5);
 //       matches = numEqualWithMemberMatcher.matches();
 //       ASSERT_EQ(matches.size(), 0);
 //     }
 ...
 //       for(auto && m : matches){
 //         auto str = m.C1->name();
 //         if(str != "C1" && str != "C2" && str != "C3" && str != "C4") FAIL() << "RingMembers Init error: unexpected ring member "
 //         << m.C1->name();
 //       }
 //     }
 // }
   ...
   TEST_F(SNCLRuntimeTest, linkedQueryUnboundTest) {
     ...
     auto matches = linkedMatcher.matches();
      ...
      for(auto m : matches){
        checkList.insert(m.S1->name());
        checkList.insert(m.S2->name());
        if(!SNCLRuntimeTest::validMatch(m)) FAIL() << "Unbound LinkedQuery found an invalid match: " << m.S1->name()
         << "-" << m.S2->name();
      }
      int i = 0;
      for(auto name : checkList) i++;
   ...
   TEST_F(SNCLRuntimeTest, linkedQuerySimpleBoundTest) {
     for(auto sn : snStore){
       ...
       if(sn->name() != "S5"){
         ASSERT_EQ(2,matches.size());
         for(auto m : matches){
           if(!SNCLRuntimeTest::validMatch(m)) FAIL() << "Bound LinkedQuery found an invalid match: "
           << m.S1->name() << "-" << m.S2->name();
         }
       }
       else{
 ...
 TEST_F(SNCLRuntimeTest, linkedQueryDoubleBoundTest) {
     for(auto s1 : snStore){
       for(auto s2 : snStore){
         ...
         if(matches.size() > 0)
           for(auto m : matches){
             if(!SNCLRuntimeTest::validMatch(m)) FAIL() << "Bound LinkedQuery found an invalid match: " << m.S1->name()
             << "-" << m.S2->name();
           }
         else{
