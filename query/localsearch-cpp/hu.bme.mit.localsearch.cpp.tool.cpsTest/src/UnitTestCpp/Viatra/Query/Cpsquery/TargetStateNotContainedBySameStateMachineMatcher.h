...
public:
	...
	std::unordered_set<TargetStateNotContainedBySameStateMachineMatch> matches() const {
		...
		{
			auto sp = TargetStateNotContainedBySameStateMachineQuerySpecification<ModelRoot>::get_plan_unbound__0(_model);
			auto exec = SearchPlanExecutor<TargetStateNotContainedBySameStateMachineFrame_0>(sp, *_context);
			
			
			for (auto&& frame : exec) {
				TargetStateNotContainedBySameStateMachineMatch match;
			
				match.transition = static_cast<::cyberPhysicalSystem::Transition*>(frame._0);
				match.target = static_cast<::cyberPhysicalSystem::State*>(frame._1);
