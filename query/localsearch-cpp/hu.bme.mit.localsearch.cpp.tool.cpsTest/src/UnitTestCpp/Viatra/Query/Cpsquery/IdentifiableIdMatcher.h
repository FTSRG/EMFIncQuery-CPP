...
public:
	...
	std::unordered_set<IdentifiableIdMatch> matches() const {
		...
		{
			auto sp = IdentifiableIdQuerySpecification<ModelRoot>::get_plan_unbound__0(_model);
			auto exec = SearchPlanExecutor<IdentifiableIdFrame_0>(sp, *_context);
			
			
			for (auto&& frame : exec) {
				IdentifiableIdMatch match;
			
				match.identifiable = static_cast<::cyberPhysicalSystem::Identifiable*>(frame._0);
				match.id = static_cast<::std::string>(frame._1);
