...
public:
	...
	std::unordered_set<AvailableGreaterThanTotalRamMatch> matches() const {
		...
		{
			auto sp = AvailableGreaterThanTotalRamQuerySpecification<ModelRoot>::get_plan_unbound__0(_model);
			auto exec = SearchPlanExecutor<AvailableGreaterThanTotalRamFrame_0>(sp, *_context);
			
			
			for (auto&& frame : exec) {
				AvailableGreaterThanTotalRamMatch match;
